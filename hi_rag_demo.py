...
async def embedding_function(texts: list[str]) -> np.ndarray:
    """Use Ollama embedding model via direct HTTP calls for reliability"""
    import aiohttp
    import asyncio
    ...
    # Use direct HTTP calls to Ollama instead of OpenAI client
    async with aiohttp.ClientSession() as session:
        for text in texts:
            try:
                # Direct call to Ollama's embedding API
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        embedding = data.get("embedding", [0] * 1024)
...
async def glm_llm_function(
    ...
    """Function to call the GLM model via direct Ollama API calls"""
    import aiohttp
    ...
    try:
        # Use direct HTTP calls to Ollama API instead of OpenAI client
        async with aiohttp.ClientSession() as session:
            async with session.post(
                "http://localhost:11434/api/chat",
                json={
                    "model": "llama3.2:3b",  # Using faster smaller model
                    "messages": messages,
                    "stream": False
                },
